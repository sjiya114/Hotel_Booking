                ...
                <div className={`flex flex-row ${!open && 'px-8'} justify-between ${!open && 'pt-4'}  min:md:px-20`}>
                    ...
                    {open && <div className='absolute flex bg-white flex-col w-screen h-screen'>
                        ...
                        <div className='flex flex-col text-black pt-20 text-center space-y-4'>
                            ...
                            <div> <a href='/rooms' className='hover:underline'>Hotels</a></div>
                            <div><a className='hover:underline'>Experience</a></div>
                            <div><a className='hover:underline'>About</a></div>
                            {user &&  <div> <a  onClick={()=>{!isOwner?setShowHotelReg(true):nav("/owner")}}       className='border-1 border-white px-2 py-2 rounded-2xl'>{isOwner ? 'Dashboard' : 'List Your Hotels'}</a></div>}
                            <div>
                            ...
                            <div>
                                ...
                                {isopen &&
                                    <div className='text-black bg-white flex flex-col px-2 py-2 rounded-lg    shadow-2xs shadow-white'>
                                        <div><a className='hover:underline'> <button onClick={()=>nav("/my-bookings")}>My Bookings</button></a></div>
                                        <div><a className='hover:underline cursor-pointer'   > <button onClick={handleLogout}>LogOut</button></a></div>
                                    </div>
                                }
                            </div>
